...
export default function Selprofile() {
    ...
    const handleSubmit = async () => {
        ...
        const response = await fetch(base_url + "/api/user_seller_profile", {
            ...
            body: JSON.stringify({ token: localStorage.getItem("sellerauthtoken"), userSellerType: "seller" })
        });
    ...
    const updateprofile = async (b64string) => {
        ...
        const response = await fetch("https://room-rover-app-backend-mern.onrender.com/api/update_profile", {
            ...
            body: JSON.stringify({ token: localStorage.getItem("sellerauthtoken"), name: profile.name, address: "", base64: b64string, userSellerType: "seller" })
        });
        const json = await response.json()
        if (json.success) {
            handleSubmit();
